...
/**
 * Pure dice engine. No React, no storage — just functions over a RollSpec.
 *
 * Randomness is injectable (`rng`) so results are deterministic in tests and so
 * a future server could replace it with a seeded/shared source.
 */
...
function rollGroup(group: DiceGroup, rng: RNG): DieRoll[] {
  const rolls: DieRoll[] = [];
